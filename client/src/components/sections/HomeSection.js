...
const SLOTS = [
  {
    ...
    icon: '🌅',
    hours: [5, 11],   // 5 AM – 11:59 AM
    prompt: 'How did you wake up feeling today?',
    sub: 'Starting the day with awareness sets a positive tone.',
    color: '#f59e0b',
  },
  {
    key: 'afternoon',
    ...
    icon: '☀️',
    hours: [12, 17],  // 12 PM – 5:59 PM
    prompt: 'How are you holding up mid-day?',
    sub: 'A quick check-in helps you catch stress before it builds.',
    color: '#6366f1',
  },
  {
    key: 'evening',
    ...
    icon: '🌙',
    hours: [18, 23],  // 6 PM – 11:59 PM
    prompt: 'How did today treat you overall?',
    sub: 'Reflecting at night helps you process and wind down.',
    color: '#8b5cf6',
  },
];
...
// Determine which slot is active right now
function getCurrentSlot() {
  const h = new Date().getHours();
  return SLOTS.find(s => h >= s.hours[0] && h <= s.hours[1]) || null;
}

// Friendly time label for a slot
function slotTimeLabel(slot) {
  const map = { morning: '5 AM – 12 PM', afternoon: '12 PM – 6 PM', evening: '6 PM – 12 AM' };
  return map[slot] || '';
}
...
export default function HomeSection({ navTo }) {
  ...
  // Mood check-in state
  const [todayLogs, setTodayLogs]     = useState([]);   // slots logged today
  const [loadingMood, setLoadingMood] = useState(true);
  const [activeSlot, setActiveSlot]   = useState(null); // slot being shown
  const [selectedScore, setSelectedScore] = useState(null);
  const [note, setNote]               = useState('');
  const [submitting, setSubmitting]   = useState(false);
  const [justLogged, setJustLogged]   = useState(null); // { slot, score, label }
  ...
  // Set active slot based on current time, skip if already logged
  useEffect(() => {
    if (loadingMood) return;
    const current = getCurrentSlot();
    if (!current) { setActiveSlot(null); return; }
    const alreadyDone = todayLogs.find(l => l.slot === current.key);
    setActiveSlot(alreadyDone ? null : current);
  }, [loadingMood, todayLogs]);
  ...
  const submitMood = async () => {
    ...
    try {
      ...
      setNote('');
      fetchTodayMoods(); // refresh
    } catch (e) {
      // 409 = already logged this slot
  ...
  // Next upcoming slot (not yet logged, not current)
    ...
    <section className="section active" id="section-home">
      ...
      <div className="mood-checkin-card" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 0 }}>
            ...
            {SLOTS.map(s => {
              ...
              return (
                <div key={s.key} style={{
                  ...
                  {done && <span style={{ marginLeft: 2 }}>✓</span>}
                </div>
              );
            })}
          </div>
        ...
        {!allDone && justLogged && !activeSlot && (
          <div style={{ padding: '12px 0 4px', color: 'var(--text-muted)', fontSize: 14 }}>
            ...
            {nextSlot && (
              <span> Next check-in: <strong style={{ color: nextSlot.color }}>{nextSlot.icon} {nextSlot.label}</strong> at {slotTimeLabel(nextSlot.key).split('–')[0].trim()}</span>
            )}
          </div>
        )}
        ...
        {!allDone && !activeSlot && !justLogged && !loadingMood && (
          <div style={{ padding: '16px 0', color: 'var(--text-muted)', fontSize: 14 }}>
            {(() => {
              ...
              if (nextSlot) return <span>{nextSlot.icon} Next check-in: <strong style={{ color: nextSlot.color }}>{nextSlot.label}</strong> — available {slotTimeLabel(nextSlot.key)}</span>;
              return <span>✓ All check-ins for today are complete. See you tomorrow!</span>;
            })()}
          </div>
        )}

        {/* Active slot — show check-in UI */}
        {!allDone && activeSlot && (
          <div>
            ...
            {selectedScore && (
              <div style={{ marginBottom: 14 }}>
                <input
                  ...
                  placeholder={`Optional: what's on your mind? (${activeSlot.sub})`}
                  value={note}
        ...
        {/* Progress dots */}
        {!loadingMood && (
          <div style={{ display: 'flex', gap: 6, marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
            ...
            {SLOTS.map(s => {
              ...
              return (
                <div key={s.key} title={log ? `${s.label}: ${opt?.label}` : `${s.label}: not logged yet`}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 4,
                    fontSize: 12, color: log ? s.color : 'var(--text-dim)',
      ...
      <div className="pillars-section">
        ...
        <div className="pillars-grid">
          ...
          <div className="pillar-card" onClick={() => navTo('sos')}>
            <div className="pillar-icon-wrap sos-gradient"><span className="pillar-icon">🆘</span></div>
            <div className="pillar-badge">Professional</div>
            <h3 className="pillar-title">SOS Crisis</h3>
            <p className="pillar-desc">Immediate access to licensed therapists and live clinic availability. For when you need professional help right now.</p>
            <ul className="pillar-features"><li>✓ Licensed therapists</li><li>✓ Live clinic slots</li><li>✓ Emergency response</li></ul>
            <button className="pillar-cta sos-cta">Get Help Now →</button>
          </div>
        </div>
      </div>
